...
std::string P2PNode::msg(const std::string&type,const std::string&payload) {
    return payload.empty()?
        "{\"type\":"+json_escape(type)+"}":
        "{\"type\":"+json_escape(type)+",\"payload\":"+json_escape(payload)+"}";
    }

std::string P2PNode::msg_num(const std::string&type,const std::string&payload) {
    return "{\"type\":"+json_escape(type)+",\"payload\":"+payload+"}";
    }
...
void P2PNode::handle(em_socket_t fd,std::string remote_host,int remote_port) {
        ...
        em_set_socket_timeout(fd,SO_RCVTIMEO,P2P_HANDSHAKE_TIMEOUT_SEC);
        em_set_socket_timeout(fd,SO_SNDTIMEO,P2P_HANDSHAKE_TIMEOUT_SEC);

        auto h=recv_frame(fd);
        if(!h)return;

        json_object*ho=json_tokener_parse(h->c_str());
        if(!ho||!valid_hello(ho)){
            if(ho)json_object_put(ho);
            return;
        }
        ...
        em_set_socket_timeout(fd,SO_RCVTIMEO,P2P_IDLE_TIMEOUT_SEC);

        for(;;){
            auto fr=recv_frame(fd);
            if(!fr)break;

            json_object*o=json_tokener_parse(fr->c_str());
            ...
            json_get_string(o,"payload",payload);

            json_object*po=nullptr;
            size_t start=0;

            if(json_object_object_get_ex(o,"start",&po)&&
               json_object_is_type(po,json_type_int)){
            ...
            if(type=="gettip") {
                ...
                if(!send_frame(fd,msg("tip",c.back().json())))break;
            }
                ...
                size_t from=std::min(start,c.size());
                size_t end=std::min(c.size(),from+MAX_BLOCKS_RESPONSE);
                ...
                if(!send_frame(fd,msg("headers",out.str())))break;
            }
            else if(type=="getblocks") {
                if(!send_blocks(fd,start))break;
            }
                ...
                if(cj.empty()){
                    send_frame(fd,msg("error","chain_too_large"));
                    break;
                }

                if(!send_frame(fd,msg("chain",cj)))break;
            }
            else if(type=="ping") {
                if(!send_frame(fd,msg("pong")))break;
            }
            else if(type=="submitblock") {
                Block b;
                bool ok=parse_block(payload,b)&&bc.accept_block(b);

                if(!send_frame(fd,msg(ok?"accepted":"rejected")))break;
            }
            else if(type=="bye") {
                break;
            }
            else {
                if(!send_frame(fd,msg("error","unknown_message")))break;
            }
        }
    }
...
em_socket_t P2PNode::connect_peer() {
        ...
        std::string ps=std::to_string(peer_port);

        if(getaddrinfo(peer_host.c_str(),ps.c_str(),&hints,&res)!=0)
            return EM_INVALID_SOCKET;

        em_socket_t fd=EM_INVALID_SOCKET;
...
bool P2PNode::handshake(em_socket_t fd) {
        ...
        if(o){
            ...
            ok=json_get_string(o,"type",type)&&
               type=="hello"&&
...
void P2PNode::sync_once() {
        ...
        auto local=bc.snapshot_chain();

        size_t start=local.size();
        ...
        while(true){
            ...
            if(!request_range(fd,start,chunk))
                break;
            ...
            if(chunk.size()<MAX_BLOCKS_RESPONSE)
                break;
        }

        if(!remote.empty()){

            std::vector<Block>candidate=local;

            if(remote.front().index==
               static_cast<int>(candidate.size())){
                ...
                while(true){
                    ...
                    if(ch.size()<MAX_BLOCKS_RESPONSE)
                        break;
                }
        ...
        send_frame(fd,msg("bye"));

        em_socket_close(fd);
    }

P2PNode::P2PNode(Blockchain&b,int lp):
    bc(b),
    listen_port(lp) {
        em_socket_init();
    }
...
void P2PNode::serve() {
        ...
        if(server_fd==EM_INVALID_SOCKET)
            throw std::runtime_error("P2P socket failed");
        ...
        if(!em_set_socket_int(
            ...
            server_fd=EM_INVALID_SOCKET;
            throw std::runtime_error("P2P setsockopt failed");
        }
