...
class P2PNode {
    Blockchain&bc; int listen_port=0; std::string peer_host; int peer_port=0; std::atomic<bool>stop{false}; em_socket_t server_fd=EM_INVALID_SOCKET;
    std::mutex peers_mu;
...
bool send_blocks(em_socket_t fd,size_t start);
void handle(em_socket_t fd,std::string remote_host,int remote_port);
em_socket_t connect_peer();
bool handshake(em_socket_t fd);
bool request_range(em_socket_t fd,size_t start,std::vector<Block>&out);
void sync_once();
